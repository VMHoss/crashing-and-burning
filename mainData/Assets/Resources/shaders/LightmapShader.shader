Shader "XformShaders/UnlitLightmapFog" {
Properties {
...
{
  lowp vec4 tmpvar_1;
  tmpvar_1 = ((texture2D (_MainTex, xlv_TEXCOORD0) * texture2D (_LightMap, xlv_TEXCOORD1)) * _MainColor);
...
lowp vec4 frag( in Input IN ) {
    lowp vec4 c = texture( _MainTex, IN.uv1);
    lowp vec4 cl = texture( _LightMap, IN.uv2);
    #line 328
    lowp vec4 tint = vec4( 1.0, 0.0, 0.0, 0.0);
    highp float LightmapOpacity = 0.5;
    return ((c * cl) * _MainColor);
}
...
void main() {
    ...
    Input xlt_IN;
    xlt_IN.pos = vec4(0.0);
    xlt_IN.uv1 = vec2(xlv_TEXCOORD0);
    xlt_IN.uv2 = vec2(xlv_TEXCOORD1);
